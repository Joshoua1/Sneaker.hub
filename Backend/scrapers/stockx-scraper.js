...
module.exports = {
    getProductsAndInfo: async function (key, count, callback) {
        // Initialize products array at the beginning
        var products = [];
        ...
        try {
            ...
            if (products.length == 0 || numOfShoes == 0) {
                callback(new Error('Product Not Found'), []);  // Return empty array instead of null
            } 
            else {
            ...
            console.log(err);
            callback(err, []);  // Return empty array instead of undefined products
        }
    },
